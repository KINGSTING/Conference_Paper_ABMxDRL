...
The \textit{Status Quo} strategy distributed funds equally (P53,571/barangay), acting as a ``dilution mechanism.'' Small communities like Mati achieved compliance via social momentum, but critical zones like Poblacion stagnated at 0.33\%.
...
The \textit{HuDRL} agent outperformed all manual strategies, achieving \textit{92.8\%} terminal compliance. The AI discovered \textit{Sequential Saturation}, concentrating 51\%--69\% of the total municipal budget on single barangays sequentially (e.g., Demologan in Q2, Poblacion in Q8-12). This ensured intervention intensity exceeded resistance thresholds, prioritizing sequential efficacy over simultaneous fairness.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{l c c c c}
        ...
        \textit{Adaptive (HuDRL)} & \textit{10.7\%} & \textit{45.3\%} & \textit{72.2\%} & \textit{92.8\%} \\
        \bottomrule
    \end{tabular}
\end{table}
...
A Sobol Global Sensitivity Analysis (GSA) revealed the \textit{Cost of Effort ($c_{\text{effort}}$)} as the primary driver, accounting for \textit{83\%} of variance. This provides evidence for the ``Convenience Hypothesis,'' where structural deficiencies override awareness. \textit{Social Norms ($w_{sn}$)} acted as a force multiplier, while \textit{Attitude ($w_a$)} showed near-zero sensitivity, quantifying the ``Value-Action Gap'' where awareness alone fails without structural support.

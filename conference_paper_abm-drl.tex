...
Agent-Based Modeling functions as a ``virtual laboratory,'' uniquely equipped to capture population heterogeneity and economic disparities that macro-level System Dynamics models miss \cite{taraghi2025}. While ABM provides the environment, Deep Reinforcement Learning (DRL) is essential for autonomously discovering optimal policies within high-dimensional state spaces. Currently, DRL is extensively utilized for physical waste sorting and collection routing \cite{zheng2022b, zhang2021}. However, its application in optimizing governance resources to actively influence human behavior remains critically under-explored.
...
Directing human behavior introduces the ``Sparse Reward Problem,'' where delayed causal effects cause standard AI agents to fail \cite{zheng2022b}. To overcome this, the MDP utilizes a composite reward function ($R_{total}$):
...
\subsection{Comparison on Heuristic vs. Non-Heuristic PPO}
